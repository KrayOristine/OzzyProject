import {
  MappedData,
  MappedDataRow,
} from "@/mdx-m3/utils/mappeddata.ts";
import {
  camelCase,
  pascalCase,
  war3ToDefaultTS,
...
function generateProps(metaData: MappedData, weStrings: MappedDataRow): Prop[] {
  ...
  for (const [id, row] of Object.entries(metaData.map)) {
    const field = row.string("field") as string;
    const type = row.string("type") as string;
    const tsType = war3TypeToTS(type);
    const displayName = row.string("displayname") as string;
    const profile = row.string("slk") === "Profile"; // Used for strings.
    // TODO: notspecific is a blacklist
    const specific = row.string("usespecific")/* || row.string("notspecific")*/; // Used for abilities.
    let name = camelCase(weStrings.string(displayName.toLowerCase()) as string);
    ...
    if (conflict) {
      name += row.string("effecttype") as string;
    }
...
function generateTSInterface(name: string, props: Prop[]): string {
  ...
  const outBaseProps = props
    .map(
      (prop) =>
        `{ id: '${prop.id}', name: '${prop.name}', type: '${prop.type}', netsafe: '${prop.row.string("netsafe") === "1"}' } as const`
    )
    .join(", ");
...
function generateTSAbilityInterfaces(
  ...
  for (const object of Object.values(objects)) {
    const id = object["oldId"] as string;
    const lookupId = (object['lookupId'] !== undefined ? object['lookupId']: object["oldId"]) as string;
...
export async function objectDataGenerator({
  ...
  const unitProps = unitAndItemProps.filter(
    (prop) =>
      prop.row.string("useunit") === "1" ||
      prop.row.string("usehero") === "1" ||
      prop.row.string("usebuilding") === "1"
  );
  const itemProps = unitAndItemProps.filter(
    (prop) => prop.row.string("useitem") === "1"
  );
