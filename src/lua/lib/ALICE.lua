...
do
  ...
  local coord = {
    ...
    unitZ         = setmetatable({},
      { __index = function(self, key)
        self[key] = GetTerrainZ(key.x[key.anchor], key.y[key.anchor]) + GetUnitFlyHeight(key.anchor) + key.zOffset
        return self[key]
      end }),
  ...
  CreateBinds = function(self, bindToBuff, bindToOrder)
    ...
    elseif HandleType[self.anchor] == "unit" then
      ...
      Identifier2String(self.identifier) .. " to a buff or order, but that actor doesn't have a unit host or anchor.")
    end
  end
  ...
  GetDescription = function(self)
    ...
    if self.bindToOrder then
      description = description + "\n|cffffcc00Bound to order:|r " + OrderId2String(self.bindToOrder) + " |cffaaaaaa(current order = " +
      OrderId2String(GetUnitCurrentOrder(self.anchor)) + ")|r"
    end
    ...
    if A then
      if B == SELF_INTERACTION_ACTOR then
        if A.periodicPair then
          warning = "\n|cffff0000Error:|r ALICE Cycle crashed during last execution. The crash occured during a periodic callback with function |cffaaaaff" ..
          Function2String(crashingPairOrCallback[0x8]) .. "|r. The interaction has been disabled."
        else
          warning = "\n|cffff0000Error:|r ALICE Cycle crashed during last execution. The identifier of the actor responsible is "
              ...
              "|r. Unique number: " ..
              A.unique ..
              ". The crash occured during self-interaction with function |cffaaaaff" .. Function2String(crashingPairOrCallback[0x8]) .. "|r. The interaction has been disabled."
        end
      else
        warning = "\n|cffff0000Error:|r ALICE Cycle crashed during last execution. The identifiers of the actors responsible are "
            ...
            "|cffffcc00" ..
            Identifier2String(A.identifier) ..
            "|r and |cffaaaaff" .. Identifier2String(B.identifier) .. "|r. Unique numbers: " .. A.unique .. ", " .. B.unique .. ". The pair has been removed from the cycle."
      end
      ...
      warning = "\n|cffff0000Error:|r ALICE Cycle crashed during last execution. The crash occured during a delayed callback with function |cffaaaaff" ..
      Function2String(crashingPairOrCallback.callback) .. "|r."
    end
  ...
  local function Main(nextStep)
    ...
    if debug.benchmark then
      ...
      "ms|r, actors: " .. #actorList .. ", pairs: " .. numPairs[currentCounter] + numEveryStepPairs .. ", cell checks: " .. numCellChecks[currentCounter])
    end
  ...
  function ALICE_Statistics()
    ...
    for __, functionType in ipairs(sortedKeys) do
      if (100 * functionCount[functionType] / countActivePairs) > 0.1 then
        ...
        "\x25 |cffffcc00" .. Function2String(functionType) .. "|r |cffaaaaaa(" .. functionCount[functionType] .. ")|r"
      end
    end
